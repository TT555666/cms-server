...
import { awaitWrap } from '@src/utils';
import redisConfig from '@src/config/redis.config';
import kafkaConfig from '@src/config/kafka.config';

const redisSeckill = redisConfig;
const kafkaDetailConfig = kafkaConfig;
const Producer = kafka.Producer;
const kafkaClient = new kafka.KafkaClient({
  kafkaHost: kafkaDetailConfig.kafkaHost,
});
...
export class SeckillService {
  ...
  async setRemainCount(remainCount: number) {
    ...
    //更新redis的counter数量
    const [execError, replies] = await awaitWrap(
      this.seckillRedisClient
        .multi()
        .set(seckillCounterKey, remainCount)
        .get(seckillCounterKey)
        .exec(),
    );
    execError && this.logger.error(execError);
    if (execError) return execError;
